...
from premute.dropblock import DropBlock
...
# block(self.inplanes, planes, stride, downsample, drop_rate, drop_block, block_size)
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.0, drop_block=False, block_size=1):
        ...
        self.downsample = downsample  # 残差连接
        self.stride = stride
    ...
    def block_forward_para(self, x, params, base, mode, modules, downsample=False):
        '''the forward function of BasicBlock give parametes'''
        self.num_batches_tracked += 1
        ...
        out = F.conv2d(out, params[base + 'conv4.weight'], stride=(1, 1), padding=(1, 1))
        out = F.batch_norm(out, weight=params[base + 'bn4.weight'], bias=params[base + 'bn4.bias'],
                           running_mean=modules['bn4'].running_mean,
                           running_var=modules['bn4'].running_var, training=mode)
...
class ResNetMAML(nn.Module):
    ...
    def __init__(self, inplanes, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=10):
        ...
        self.layer4 = self._make_layer(block, 512, stride=2, drop_rate=drop_rate, drop_block=True, block_size=5)
        if avg_pool:
    ...
    def forward(self, x, params=None, embedding=False):
        ...
        x = self.layer1[0].block_forward_para(x, params, 'layer1' + '.0.', self.training,
                                              self._modules['layer1']._modules['0']._modules, True)
        x = self.layer2[0].block_forward_para(x, params, 'layer2' + '.0.', self.training,
                                              self._modules['layer2']._modules['0']._modules, True)
        x = self.layer3[0].block_forward_para(x, params, 'layer3' + '.0.', self.training,
                                              self._modules['layer3']._modules['0']._modules, True)
        x = self.layer4[0].block_forward_para(x, params, 'layer4' + '.0.', self.training,
...
if __name__ == '__main__':
    ...
    # a,b = zip(*params.items())
    # print(type(a))
    x = torch.rand(3, 1, 84, 84)
    model = ResNetMAML(inplanes=1)
    pred = model(x)
